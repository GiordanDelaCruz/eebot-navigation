...
LEFT_TURN     EQU   3
RIGHT_TURN    EQU   4
REV_TURN      EQU   5                     ; LEFT_TIMED & RIGHT_TIMED are the states that allow 
...
;***************************************************************************************************         
;*                         START OF TEMPORARY CODE TO READ SENSORS                                 *
;***************************************************************************************************
...
;***************************************************************************************************
;*                         END OF TEMPORARY CODE TO READ SENSORS                                   *
;*************************************************************************************************** 
...
; -------------------------------------------------------------------------------------------------
DISPATCHER        JSR   CHECK_START                           ; Beginning of the Dispatcher
                  RTS
                
CHECK_START       CMPA  #START                                ; Check if the robot is in the 
                  BNE   CHECK_FORWARD                         ; START state, else proceed to 
                  JSR   START_ST                              ; Check if the robot is in the 
                  RTS                                         ; FORWARD state

CHECK_FORWARD     CMPA  #FWD                                  ; Check if the robot is in the 
                  BNE   CHECK_STOP                            ; FORWARD state, else proceed to 
                  JSR   FWD_ST                                ; Check if the robot is in the 
                  RTS                                         ; ALL_STOP state

CHECK_STOP        CMPA  #ALL_STOP                             ; Check if the robot is in the
                  BNE   CHECK_LEFT_TURN                       ; ALL_STOP state, else proceed to  
                  JSR   ALL_STOP_ST                           ; Check if the robot is in the  
                  RTS                                         ; LEFT_TURN state

CHECK_LEFT_TURN   CMPA  #LEFT_TURN                            ; Check if the robot is in the 
                  BNE   CHECK_RIGHT_TURN                      ; LEFT_TURN state, else proceed to
                  JSR   FULL_LEFT                             ; Check if the robot is in the 
                  RTS                                         ; RIGHT_TURN state

CHECK_RIGHT_TURN  CMPA  #RIGHT_TURN                           ; Check if the robot is in the 
                  BNE   CHECK_REV_TURN                        ; RIGHT_TURN state, else proceed to 
                  JSR   FULL_RIGHT                            ; Check if the robot is in the 
                  RTS                                         ; REV_TURN state
                  
CHECK_REV_TURN    CMPA  #REV_TURN                             ; Check if the robot is in the 
                  BNE   CHECK_LEFT_ALIGN                      ; REV_TURN state, else proceed to 
                  JSR   REV_TURN_ST                           ; Check if the robot is in the 
                  RTS                                         ; LEFT_ALIGN state
                  
CHECK_LEFT_ALIGN  CMPA  #LEFT_ALIGN                           ; Check if the robot is in the 
                  BNE   CHECK_RIGHT_ALIGN                     ; LEFT_ALIGN state, else proceed to 
                  JSR   LEFT_ALIGN_EXIT                       ; Check if the robot is in the 
                  RTS                                         ; RIGHT_ALIGN state

CHECK_RIGHT_ALIGN CMPA  #RIGHT_ALIGN                          ; Check if the robot is in the 
                  BNE   INVALID_STATE                         ; RIGHT_ALIGN state, else proceed to
                  JSR   RIGHT_ALIGN_EXIT                      ; Check if the robot is in the 
                  RTS                                         ; INVALID state
...
; -------------------------------------------------------------------------------------------------
; Start subroutine:                                                                                |
;   - Checks front bumper release then init fwd if it's 0V (i.e pressed down),                     |
...
; -------------------------------------------------------------------------------------------------
; Forward Subroutine:                                                                              |
;   - Checks if front bumper is hit and then perfroms a 180 degree turn                            |
;   - Checks if rear bumper is hit and then performs a stop                                        |
;   - Checks if the robot is on path with the black electrical tape utilizing sensors E-F          |
;     and turns accordingly                                                                        |
; -------------------------------------------------------------------------------------------------               
FWD_ST            BRSET   PORTAD0, %00000100, NO_FWD_BUMP           ; Checks if bow bumper is hit, |                          
                  MOVB    #REV_TURN, CRNT_STATE                     ; if true, enter the           |                        
                                                                    ; REV_TURN state               |               
                  ...
                  LBRA    EXIT                                                                    ;|
                                                                                                  ;|
NO_FWD_BUMP       BRSET   PORTAD0, %00001000, NO_FWD_REAR_BUMP      ; Checks if the stern bumper,  | 
...
; OVERVIEW:                                                                                       ;|
;----------                                                                                        |
;   Read the BOW, MID, PORT, STBRD, & LINE sensors and add a small amount of variance to it.       |
;   This will enable the robot to attain a wider range of values where to correct any              |
;   errors/miscalculations in terms of following the black electrical tape.                        |
;                                                                                                  |
;   After, compare the present values of the sensors (with the addition of varaince), is less      |
;   than or greater than the initial values, and determine whether the robot should either turn    |
;   left, right, or straighten out.                                                                |
                                                                                                  ;|
...
;   - Check if the BOW_SENSOR is back on the black electrical  tape, if true then                  |
;     move forward again.                                                                          |
; -------------------------------------------------------------------------------------------------                  
...
; -------------------------------------------------------------------------------------------------
; Left Align subroutine:                                                                           |
;   - After turning to the left a bit, return to moving forward again .                            |
; -------------------------------------------------------------------------------------------------                       
...
; -------------------------------------------------------------------------------------------------
; Right Align subroutine:                                                                          |
;   - After turning to the right a bit, return to moving forward again .                           |
; -------------------------------------------------------------------------------------------------                   
...
; -------------------------------------------------------------------------------------------------
; 180 Turn subroutine:                                                                             |
;   - After performing a 180 right turn, check if the bow sensor is back on the black electrical,  |
;     then turn left abit to correct its self slightly, and then go back to moving forward.        |
; -------------------------------------------------------------------------------------------------
                                                                                                  ;|
...
; -------------------------------------------------------------------------------------------------
; Stop subroutine:                                                                                ;|
;   - Checks front bumper press then moves to start state, otherwise, do nothing                  ;|
; -------------------------------------------------------------------------------------------------                         
