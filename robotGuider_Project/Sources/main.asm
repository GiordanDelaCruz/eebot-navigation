...
RIGHT_TURN    EQU   4
REV_TURN      EQU   5
LEFT_TIMED    EQU   6    ;POTENTIALLY USEFUL
RIGHT_TIMED   EQU   7    ;POTENTIALLY USEFUL
...
;Initialization
              ...
              ;JSR   INIT_SENSORS       ; Fires off the sensors and stores in BASE_[SENSOR] variables               
...
MAIN        
              ...
              LDY   #2000 ; 300 ms delay to avoid                                 |
...
tab           dc.b  "start  ",0
              ...
              dc.b  "RTurn  ",0
              dc.b  "RevTrn ",0
              dc.b  "LTimed ",0     ;POTENTIALLY USEFUL
              dc.b  "RTimed ",0     ;POTENTIALLY USEFUL
; subroutine section
;*******************************************************************

; Dispatcher
DISPATCHER        JSR   CHECK_START
                  RTS
                
CHECK_START       CMPA  #START
                  BNE   CHECK_FORWARD
                  JSR   START_ST
                  RTS

CHECK_FORWARD     CMPA  #FWD 
                  BNE   CHECK_STOP
                  JSR   FWD_ST
                  RTS

CHECK_STOP        CMPA  #ALL_STOP
                  BNE   CHECK_LEFT_TURN
                  JSR   ALL_STOP_ST
                  RTS

CHECK_LEFT_TURN   CMPA  #LEFT_TURN
                  BNE   CHECK_RIGHT_TURN
                  JSR   FULL_LEFT
                  RTS

CHECK_RIGHT_TURN  CMPA  #RIGHT_TURN
                  BNE   CHECK_REV_TURN
                  JSR   FULL_RIGHT
                  RTS
                  
CHECK_REV_TURN    CMPA  #REV_TURN
                  BNE   CHECK_TIMED_LEFT
                  JSR   REV_TURN_ST
                  RTS
                  
CHECK_TIMED_LEFT  CMPA  #LEFT_TIMED
                  BNE   CHECK_TIMED_RIGHT
                  JSR   LEFT_EXIT
                  RTS

CHECK_TIMED_RIGHT CMPA  #RIGHT_TIMED
                  BNE   INVALID_STATE
                  JSR   RIGHT_EXIT
                  RTS
...
; ---------------------------------------------------------------------------------------------
; Start subroutine:
; Checks front bumper release then init fwd if it's 0V (i.e pressed down), otherwise do nothing
; ---------------------------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------------------------
; Forward Subroutine:
; Checks if front bumper is hit and turns
; checks if rear bumper is hit and stops
; checks sensors E-F and turns accordingly
; ---------------------------------------------------------------------------------------------               
...
NO_FWD_REAR_BUMP  LDAA    SENSOR_PORT
                  ADDA    VAR_PORT
                  CMPA    BASE_PORT
                  BPL     PART_LEFT_TURN
                  BMI     NO_PORT_DET
...
; Takes line sensor value, adds a small amount to it
; then checks if sensorline is still less than its base value
; in order to give it a wider range of value where the bot
; moves forward 
NO_STBD_DET       ;SWI
                  LDAA    SENSOR_BOW
                  ADDA    VAR_BOW
...
PART_LEFT_TURN    JSR     INIT_LEFT
                  MOVB    #LEFT_TURN, CRNT_STATE
                  LDY     #6000
                  JSR     del_50us
                  BRA     EXIT
                  
PART_RIGHT_TURN   JSR     INIT_RIGHT
                  MOVB    #RIGHT_TURN, CRNT_STATE
                  LDY     #6000
                  JSR     del_50us
                  BRA     EXIT                  
                  
GO_LEFT_TIMED     JSR     INIT_LEFT
                  MOVB    #LEFT_TIMED, CRNT_STATE
                  BRA     EXIT
                  
GO_RIGHT_TIMED    JSR     INIT_RIGHT
                  MOVB    #RIGHT_TIMED, CRNT_STATE
                  BRA     EXIT
...
; ---------------------------------------------------------------------------------------------
; Turn around Subroutine
; ---------------------------------------------------------------------------------------------

REV_TURN_ST       JSR     FULL_RIGHT
                  LDAA    SENSOR_BOW
                  ADDA    VAR_BOW
                  CMPA    BASE_BOW
                  BMI     EXIT
...
;INIT_SENSORS      JSR    G_LEDS_ON ; Enable the guider LEDs
;                  JSR    READ_SENSORS ; Read the 5 guider sensors                      
;                  JSR    G_LEDS_OFF ; Disable the guider LEDs  
;                  
;                  LDAA   SENSOR_LINE
